...
| T4.6 | Rebasar el PR #44 y resolver los 5 conflictos | **HECHO** | **PR #44 MERGEADO** · `main` `fb6c9c9` → **`ba1390f`** · `docs/investigacion/2026-09-15-resolucion-conflictos-44.md` · ⚠️ **eran 7 conflictos, no 5** (crecieron al aterrizar #42 y #43) y se usó **merge en vez de `rebase`**: rebasar reescribe historia pública y exige force-push sobre un PR abierto · los 7 documentados uno por uno · **el más delicado, C5**: `main` seguía diciendo «secretos en Railway» y **el #44 ya lo había corregido** porque fue él quien lo apagó — ahí ganó el #44, conservando su tabla de la contradicción (404 → **502 con `x-railway-fallback`** → 404) · **riesgo del plan verificado**: el escape de fórmulas sigue cubriendo **todas** las escrituras `USER_ENTERED` tras la extracción · **5 suites** de endurecimiento (el plan decía 4) y `verificar_endurecimiento.py` con **15/15 guardas en rojo al reintroducirles su defecto** · 2 tests tocados con su motivo escrito (tope de `app.py` 3,400→3,800: +515 líneas de **Python**, no HTML) · gates `security-reviewer` + `python-reviewer` sin CRITICAL ni HIGH · **baseline 1,193 passed, 2 skipped** · **0 PR abiertos** | 2026-09-16 |
| T4.7 | Desplegar y verificar: a11y, responsive y rendimiento | **HECHO** | `docs/investigacion/2026-09-15-verificacion-produccion-plan4.md` · **DESPLEGADO** `8bac782` → **`edda166`** (87 commits) con autorización del owner · ⚠️ **hizo falta un paso que el comando NO cubre**: copiar la plantilla del compose, porque la copia viva es la que manda y el healthcheck vive ahí (verificado byte a byte idéntica salvo ese bloque, y respaldada antes) · `--build` obligatorio: sin **`tzdata`** el panel **no arranca** (riesgo R3) · **CE8 ✅** contenedor `healthy`, `RestartCount=0` · **CE4 ✅ 0 hallazgos**, foco visible en todas las paradas · **CE5 ✅ 0 desbordes en 18 combinaciones** (3 superficies × 6 anchos) · **CE6 ⚠️ intercambio declarado:** CLS mejora en todo —tablero **0.1924 → 0.0358**, peor de 18 **0.0858**, ninguna sobre 0.1— pero el **LCP del tablero se triplicó** (132 → 416 ms) por la propia extracción: 12 recursos donde antes todo iba en línea. Sigue 6× bajo el objetivo · **0 peticiones a CDN** · ⚠️ un valor atípico de **6,064 ms** en muestra única resultó ser 416 con mediana de 3 · las 18 capturas de producción se **apartaron al respaldo** (datos de negocio) y las 15 versionadas se regeneraron con la herramienta del proyecto | 2026-09-16 |
| T4.8 | Cierre: sistema documentado, PROGRESO y relevo | **HECHO** | `docs/diseno/sistema.md` **244 → 325 líneas**: **§6.0 la tabla de duraciones y curvas** que faltaba (cierra el hueco **M9** de la auditoría de T4.1) con el criterio de cuándo usar cada token, y **§6bis** con cómo añadir un componente, cómo hacer un estado de carga, **lo que no se toca sin medir** (5 cosas, cada una con el CLS que costó) y la **deuda conocida** de `dashboard.css`/`formulario.css` explicada para que nadie la fuerce sin darse cuenta de que cambiaría el render · `CLAUDE.md` apunta al sistema como lectura previa a tocar el panel · `RUNBOOK.md` estrena «Tocar el panel sin romper el diseño» con los 3 comandos de verificación y los dos avisos (las capturas no llevan datos de clientes; un atípico puede ser 15× la mediana) · índice a **2/4 planes** y relevo a **Plan 3 · T3.0** | 2026-09-17 |
