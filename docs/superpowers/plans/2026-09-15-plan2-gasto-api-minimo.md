...
| T2.5 | Tope duro por corrida y medidor vivo | **HECHO** | El tope **ya tenía 4 tests** (`TestTopeDePresupuesto`) y se verificaron **por mutación**: desactivar el tope y quitarle la causa legible ponen los dos en rojo. **Medidor vivo: comprobación nueva en `auditar_estados_importador.py`** — el `m-llamadas` de la pantalla contra el `medidor` del backend, **9/9 en local y contra producción**, y mutado (publicar ceros → **7 estados en rojo**). RUNBOOK con la calibración | **No se duplicaron los tests de agosto: se verificaron.** 🔴 **Hallazgo:** los topes **no tienen valor por defecto** — si la variable no está en el `.env` del VPS, **el tope no existe**, y ningún endpoint lo expone. *Un tope que nadie ha confirmado que está puesto no es un tope.* Owner |
| T2.6 | Verificación A/B: menos llamadas, mismos prospectos | **HECHO** | `docs/investigacion/2026-09-15-costo-places-despues-plan2.md` + `tools/verificar_ab_recortes.py` — **CE2: 18 → 13 Text Search (−28 %). CE3: diff vacío.** Y el cero vale porque el mismo diff marca **40 perdidos** con un recorte absurdo. Caché: **80 → 0** en la segunda corrida | **Alcance aclarado:** el ADR decidió no migrar, así que no hay «config nueva»; el A/B se aplica a los **dos recortes de agosto**, con su método. Confirma por otra vía que `MAX_VARIACIONES_SIN_APORTE` **aporta cero**. 🪤 Mi arnés dijo primero que **la caché no ahorraba nada** — pisaba la ruta de caché tras aplicar la config. Cuarta vez en la tanda que el instrumento falla antes que el código |
| T2.7 | Cierre: PR, despliegue, documentación y relevo final | **HECHO** | PR a `main` con CI verde. **CE6 en un solo documento**: `docs/investigacion/2026-09-15-gate-owner-gasto-en-pesos.md`. `CLAUDE.md` con el baseline al día (**1,208** en `main`, 1,221 en la rama). Índice y relevo de cierre | **No hay que desplegar**, y se mide: `git diff main..HEAD` sobre `app.py`/`static/`/`templates/`/`Dockerfile` es **vacío** — este plan añade herramientas, tests y documentación, y no toca una línea de lo que sirve el VPS |
