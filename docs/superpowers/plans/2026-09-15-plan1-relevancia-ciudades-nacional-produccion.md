...
| T1.2 | Auditoría de cobertura del catálogo por región | **HECHO** | `docs/investigacion/2026-09-15-cobertura-catalogo-ciudades.md` · medición propia sobre DENUE 05_2026 (638,861 filas, 75,726 estab., universo 2,227) reproduce exactamente las cifras de agosto · **brecha = 13.7 % de la masa ferretera nacional fuera; Sureste al 65.6 %** · 589 vs 606 explicado (17 heredados, `resolver_array_viejo`) · **D4 resuelta: umbral ≥10** vía `council` · script `tools/auditar_cobertura_ciudades.py` · medición cruda en `respaldos/2026-09-15/cobertura-t12-medicion.json` | 2026-09-15 |
| T1.3 | Cerrar la brecha de cobertura (TDD) | **HECHO** | commit `89f9ca8` · rama rebasada sobre el PR #42 (el generador no existía aquí; respaldo en tag `respaldo/pre-rebase-t13-20260915`) · **RED real**: el test falló con `Sureste: 65.7 % (6287 de 9564)` antes del cambio · umbral **20 → 10** (`generar_catalogo_ciudades.py:512`) · **606 → 1,004 municipios**, 32/32 entidades, **0 perdidas** · masa nacional **86.3 % → 93.7 %**, Sureste **65.7 % → 80.6 %**, desequilibrio **1.48× → 1.23×** · **CE1 verde** (0 de 995 fuera, test municipio a municipio) y **CE3 verde** (mínimo 14.1) · 0 nombres ambiguos en las 398 nuevas contra 82 homónimos · 3 guardas probadas por mutación · **fix preexistente**: el relleno de espacios del DENUE viajaba literal a Places · ADR **§9 anexada** (93 líneas, 0 borradas) · 3 gates **sin CRITICAL ni HIGH** · baseline **491 passed, 1 skipped** | 2026-09-15 |
| T1.4 | Verificar orden nacional y filtro por región | **HECHO** | commit `e7acf6e` · `docs/investigacion/2026-09-15-verificacion-orden-nacional.md` · **CE2 verde**: las 8 regiones parametrizadas, catálogo→endpoint→`filtrarCiudades()`, los cuatro conteos cuadran en 1,004 · **CE3 verde** por los dos extremos (mínimo 14.1; peor caso 14.1×0.60=8.46) · **CE4 APROBADO por el owner, sin reservas** · **HALLAZGO: el endpoint desempataba por NOMBRE y el ADR §7 dice CLAVE INEGI** — 103 empates, 232 ciudades, 100 posiciones distintas del catálogo; corregido sin publicar la clave · 34 tests nuevos, los tres probados por mutación · 4 gates sin CRITICAL ni HIGH, 4 MEDIUM aplicados (uno era un falso verde real: el guard del predicado se engañaba con un comentario) · el «flake» de un gate era su corrida durante la ventana RED · baseline **525 passed, 1 skipped** | 2026-09-15 |
| T1.5 | Aterrizar el PR #42 | **HECHO** | **PR #42 MERGEADO** el 2026-09-16 · `main` `82995c3` → **`8bac782`** · empujado como fast-forward (`499d41b..6404a3c`), sin tocar los SHA del #42 · **CI en verde** (barrido de secretos + pytest) · gates `python-reviewer` + `code-reviewer` **sin CRITICAL ni HIGH** (el segundo, 0 de todo) · ⚠️ **NO se usó `--squash` pese a que el plan lo pedía**: se simuló localmente y rompe el PR #43 en **11 archivos** (incluido `app.py`); con **merge commit** el #43 queda `MERGEABLE/CLEAN`, verificado por GitHub y en local · `pytest` sobre `main` → **525 passed, 1 skipped** · catálogo en `main`: 1,004 ciudades, 32/32, mínimo 14.1 · **efecto colateral registrado: el PR #44 pasó de `MERGEABLE` a `CONFLICTING` contra `main`** (`CLAUDE.md`, `app.py`, `docs/superpowers/plans/2026-08-27-indice-tanda.md`) — su rebase ya estaba previsto en Plan 4 T4.6 | 2026-09-16 |
| T1.6 | Desplegar y verificar en producción | **HECHO** | commit `407eb32` · `docs/auditoria/2026-09-15-verificacion-produccion-plan1.md` · ⚠️ **el merge NO bastaba: no existe auto-deploy** (Railway se eliminó el 2026-08-19; el invariante de los 4 planes es falso). Desplegado a mano con autorización del owner: el servidor estaba en **HEAD desacoplado** en `51520f3` (último despliegue real: **2026-08-24**), así que salieron **24 commits**. Verificado desde fuera: `/api/importador/ciudades` **404 → 200**, **1,004 ciudades**, las 8 regiones exactas, **398/398** nuevas vivas, CE3 mínimo 14.1, `clave_inegi` 0 apariciones, `RestartCount=0`, smoke `Todo OK ✅` · **CE5: parte automatizable VERDE**, faltan las 3 capturas del owner · 🔒 **hallazgo MEDIO**: el endpoint publica 8 teléfonos y 1 correo en `sin_clasificar` contra lo que promete su docstring (no introducido aquí; tras token) | 2026-09-16 |
| T1.7 | Cierre: docs, PROGRESO y relevo | **HECHO** | `CLAUDE.md` (baseline **525**, Railway retirado de 3 sitios, exposición M1 **cerrada** con 404 reverificado, lección del squash) · `docs/RUNBOOK.md` (comando de despliegue corregido —llevaba `git pull` sobre HEAD desacoplado—, lo que el smoke **no** cubre, y el aviso de PII en «sin clasificar») · índice a **1/4 planes, 8/34 tareas** con el hallazgo del auto-deploy propagado a los otros 3 planes · relevo apuntando a **Plan 4 · T4.0** | 2026-09-16 |

**Avance del plan: 3 / 8 tareas (37.5 %)**
