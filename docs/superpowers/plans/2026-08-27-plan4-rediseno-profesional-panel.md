...
| T4.6 | Sistema de movimiento | ✅ HECHA | CE6: 16 transiciones de layout → **0** (2 estaban escondidas en atributos `style=`) · la barra de progreso pasa de `width` a `transform: scaleX`, con `will-change` puesto y retirado · CE7: Chart.js apagado con `matchMedia` — dibuja en `<canvas>` y la cascada CSS no lo alcanzaba · movimiento nuevo acotado: entrada escalonada (12 filas, 25 ms) y entrada de sección; `/formulario` se queda quieto por decisión del ADR · **13/13** en navegador (`tools/verificar_movimiento.py`), preferencia emulada en las dos posiciones y cambiada en caliente · CLS sigue 9/9 < 0.1 · **690 passed, 1 skipped** (+53) · gates: `code-reviewer` 2 MEDIUM + 3 LOW y `a11y-architect` 2 HIGH + 2 MEDIUM + 1 LOW, todos corregidos · evidencia: `docs/diseno/2026-09-01-sistema-de-movimiento-t46.md` | 2026-09-01 |
| T4.7 | Rediseño del dashboard `/` | ✅ HECHA | **CE3: 31 → 0 colores literales en todo el proyecto** (eran las dos paletas de datos; el JS las lee de tokens con `getComputedStyle`) · color de gráfica por ETIQUETA y no por posición — antes bastaba con que la hoja reordenara las claves para pintar APROBADO de rojo · los 12 `div` con `onclick` pasan a `<button data-seccion>`: la navegación entera estaba fuera del alcance del teclado (SC 2.1.1) · jerarquía de 3 + 5 tarjetas con cifra dominante · encabezado fijo, orden con `aria-sort` operable por teclado, paginación anunciada · la insignia dice la edad REAL del dato (`edad_de_cache` en `app.py`), no la hora del navegador · **fallo preexistente corregido**: `editTh` condicional y `editTd` no, así que 8 de las 11 tablas mostraban cada valor una columna a la derecha de su encabezado · **24/24** en navegador (`tools/verificar_tablero.py`) · **740 passed, 1 skipped** (+50) · gates: `code-reviewer` 1 CRITICAL + 1 MEDIUM y `a11y-architect` 1 CRITICAL + 2 HIGH + 2 MEDIUM, todos corregidos · evidencia: `docs/diseno/2026-09-01-rediseno-tablero-t47.md` | 2026-09-01 |
| T4.8 | Rediseño del formulario `/formulario` | ✅ HECHA | **Captura completa solo con teclado: 11 pulsaciones frente a ~90** (contadas por el arnés, no estimadas) · **XSS almacenado demostrado en navegador y cerrado**: el nombre de tienda de la hoja ejecutaba en la pantalla del operador — misma clase que el Plan 3 cerró en el importador y que aquí nunca se hizo · los atajos que las etiquetas prometían («1 — Respondió») no existían: no había un manejador de teclado en el archivo · el foco viaja al cambiar de paso; antes caía a `<body>` y había que tabular desde el principio en cada una de las siete preguntas · el fallo de guardado deja de ser un `alert()` que perdía las respuestas: conserva el payload y reintenta con lo mismo · **12/12** en navegador (`tools/verificar_formulario.py`) · **781 passed, 1 skipped** (+41) · gates: `security-reviewer` 2 MEDIUM + 2 LOW y `code-reviewer` APPROVE con 1 MEDIUM (una regresión de accesibilidad propia: el dígito se lo quedaba quien ve la pantalla), todos corregidos · evidencia: `docs/diseno/2026-09-02-rediseno-formulario-t48.md` | 2026-09-02 |
| T4.9 | Rediseño del importador `/importador` | ✅ HECHA | **Filtro de 606 ciudades: 713.5 ms → 3.1 ms** en el peor caso de una pulsación (mismo protocolo antes y después, con calentamiento y 7 repeticiones); los chips se construyen una vez y el filtro solo oculta · **los 606 chips pasan de 0 alcanzables con teclado a un `listbox` con UNA parada de tabulación**, flechas que saltan lo oculto y `role=option`/`aria-selected` · agrupación por macro-región con conteo por grupo en vivo, ordenada por la prioridad del Plan 1 (no por nombre: eso habría destruido el ranking) · `nuevos_en_sheet` pasa a 56 px frente a 28 px de los secundarios (antes 38.4 / 28.8) · consola dentro del sistema, con pila mono declarada y alcanzable con teclado · fase visible junto al progreso · **los 4 finales se distinguen**: solo `done` celebra, cancelado y tope de gasto no son errores · **0 `alert()` y 0 `confirm()`** (eran 4 + 1) · los 50 chips que desbordaban su caja a 320 px pasan a 0 · **45/45** en navegador (`tools/verificar_importador.py`), **34 en rojo contra el código anterior** · **850 passed, 1 skipped** (+69) · gates: `code-reviewer` APPROVE (1 MEDIUM + 2 LOW), `security-reviewer` sin CRITICAL/HIGH (1 LOW) y `a11y-architect` **1 CRITICAL + 2 HIGH + 7 MEDIUM + 2 LOW** —el CRITICAL era un contraste roto por `opacity`, que ningún guarda de tokens mide—, todos corregidos · **hallazgo propio: un byte NUL crudo en el fuente** volvía el archivo «binario» para `grep`, o sea invisible para el barrido de secretos; corregido y con guarda · evidencia: `docs/diseno/2026-09-02-rediseno-importador-t49.md` | 2026-09-02 |
| T4.10 | Accesibilidad, responsive y rendimiento | ✅ HECHA | **CE10: el tablero desbordaba 461 px a 320 px y ahora 0 en los cinco anchos** — la barra lateral fija pasa a banda superior bajo 1024 px, y el desborde no era solo eso: `#main` (flex) y `.chart-box` (rejilla) traen `min-width:auto` y no podian encoger · **CE9: 17 pares de contraste bajo AA a 0**, y los tres primeros fallaban por `opacity`, no por el color — ningun guarda de CE3 los veia · **CE8: 22 campos sin nombre accesible, 4 controles inalcanzables con teclado y 7 dialogos que no se anunciaban como tales, todos corregidos** · rendimiento: Chart.js deja de venir de jsdelivr (bloqueaba el render hasta que el CDN contestara: **15.1 s** medidos) y el logo pasa de 44.8 KB a 0.6 KB; **245 KB de terceros a 0.5 KB**, las tres superficies dentro de presupuesto medido en gzip · auditor propio nuevo (`tools/verificar_accesibilidad.py`) **comprobado en las dos direcciones** · **900 passed, 2 skipped** (+50) · gates: `a11y-architect` 2 CRITICAL + 2 HIGH + 3 MEDIUM y `accessibility-tester` 5 HIGH (3 de ellos falsos, verificados contra el codigo). El hallazgo que mas cambio la tarea fue estructural: **el auditor solo miraba un estado por superficie**; al recorrer las 14 secciones y los 7 dialogos aparecieron 36 hallazgos donde daba 0 · evidencia: `docs/diseno/2026-09-03-accesibilidad-responsive-t410.md` | 2026-09-03 |
| T4.11 | Cierre: sistema documentado, PR, handoff | ✅ HECHA | `docs/diseno/sistema.md` con tokens, componentes, los cuatro estados, movimiento, accesibilidad, rendimiento y las **ocho** herramientas de verificacion · capturas del despues en `docs/diseno/despues/` (15, cinco anchos) junto a las nueve del antes: la del tablero a 320 px **medía 626 px de ancho**, o sea que el desborde estaba grabado en la propia imagen · `CLAUDE.md` corregido (app.py son **3,170**, no 3,133: las T4.5 y T4.7 le devolvieron 37 lineas de Python) · **11 de 12 criterios cumplidos**; el unico abierto es CE1, que es la decision D6 del owner · verificacion final: **900 passed, 2 skipped**, los cinco arneses en verde, auditor de accesibilidad en 0 y CLS 9/9 bajo 0.1 | 2026-09-03 |

**Avance del plan: 12 / 12 tareas (100 %)**

### 8.1 Criterios de exito al cierre (2026-09-03)

Cada uno con el comando que lo mide, no con una afirmacion.

| # | Criterio | Medida | Estado |
|---|---|---|---|
| CE1 | `app.py` < 800 lineas | `wc -l app.py` → **3,170** | ❌ **ABIERTO** — es la decision **D6** del owner, no un descuido |
| CE2 | Cero HTML embebido en Python | `grep -c render_template_string app.py` → **0** | ✅ |
| CE3 | Ningun color literal fuera de tokens | 0 archivos con hex fuera de `tokens.css` | ✅ |
| CE4 | CLS < 0.1 en las tres superficies | `medir_cls.py` → 9/9, peor **0.0329** | ✅ |
| CE5 | Esqueletos, no spinners | 0 `class="loading"` (eran 16) | ✅ |
| CE6 | Movimiento en propiedades del compositor | 0 `transition: all` reales (las 2 coincidencias son comentarios que explican por que se retiro) | ✅ |
| CE7 | `prefers-reduced-motion` respetado | En `tokens.css`, `componentes.css` y en JS para Chart.js, que el CSS no alcanza | ✅ |
| CE8 | Accesibilidad AA, todo alcanzable por teclado | `verificar_accesibilidad.py` → **0 hallazgos** recorriendo 14 secciones y 7 dialogos | ✅ |
| CE9 | Contraste verificado | 0 pares bajo AA, midiendo el color EFECTIVO (opacidad compuesta y degradados) | ✅ |
| CE10 | Responsive sin desborde en 5 anchos | 320/375/768/1024/1440 × 3 superficies → **0 desbordes** | ✅ |
| CE11 | No parece plantilla (≥4 de 10 cualidades) | El ADR compromete **7** | ✅ |
| CE12 | Comportamiento identico | **900 passed, 2 skipped** + 5 arneses de navegador en verde | ✅ |

**11 de 12.** El unico abierto es CE1, y no por falta de trabajo: extraer el HTML bajo
`app.py` un 50 % (6,368 → 3,170) y bajar de 800 exige trocear el Python en modulos, que este
plan declaro fuera de alcance desde su §0.3.
