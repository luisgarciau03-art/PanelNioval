...
| T3.3 | Test que falla (RED) | **HECHO** | `tests/test_huella_despliegue.py` — **6 tests**. RED exacto: `ImportError: cannot import name 'veredicto' from 'tools.huella_despliegue'`. **Objeto cambiado por T3.2**: no hay defecto de conteo que reproducir (80 tests lo vigilan y el código llevaba arreglado desde agosto); lo que no tenía guarda era **el despliegue rancio** | **CE2 cerrado.** Las 3 guardas probadas **por mutación**: marcador fantasma, marcador perdido y detección apagada — las tres en rojo con su defecto, archivo restaurado con sha256 idéntico |
| T3.4 | Fix (GREEN) | **HECHO** | `veredicto()` extraída a función pura y probada; **6 passed**; suite **1,193 → 1,199**. Verificado contra producción: `H1 DESCARTADA` + marcador posterior | El plan ya lo previó: *«si es H1, aquí no hay código: arreglar el despliegue y dejar escrito en RUNBOOK.md cómo se detecta un VPS rancio»*. El despliegue se arregló el 16-sep (T1.6); aquí queda la **guarda** y la sección nueva del RUNBOOK |
| T3.5 | Auditoría de las pantallas de carga | **HECHO** | `docs/investigacion/2026-09-15-auditoria-estados-carga.md` + `tools/auditar_estados_importador.py` + **9 capturas** — **9 escenarios, 0 afirmaciones falsas**. Auditor verificado en la otra dirección: **3 defectos reales de agosto reintroducidos, los 3 detectados** | **Los estados son SIETE, no seis**: `interrumpido` (SIGTERM) no estaba en la lista del plan, y es justo el que aparece al recargar tras un reinicio. Y **Telegram es un octavo canal**: los dos hallazgos de agosto (#17575, #17826) vivían ahí, y hoy están cerrados |
| T3.6 | Corregir los estados que mienten | **SE SALTA** | Los 9 escenarios coinciden con el backend (T3.5). La fila se conserva, como manda el plan | No hay mentira que corregir. Si T3.7 encuentra una **en producción**, esta tarea se reabre |
| T3.7 | Verificación de extremo a extremo, en producción | 🔴 **BLOQUEADA** | `docs/investigacion/2026-09-15-verificacion-produccion-plan3.md` + **9 capturas contra el panel desplegado** — **CE5 verde: 9/9 coinciden**. Telegram coincide con la UI. El despliegue **no hace falta**, y se mide: `git diff main..HEAD` sobre `app.py`/`static/`/`templates/` es **vacío** | 🔴 **CE3 bloqueado por el owner**: la corrida real necesita credenciales de Google (ausentes aquí), factura Places y escribe en `LISTA DE CONTACTOS` con respaldo previo. **Es el mismo gate que agosto dejó abierto el 27-ago.** Receta exacta en §5.1 del documento |
| T3.8 | Cierre: documentación, PROGRESO y relevo | **HECHO** | Expediente ampliado con **§7bis «Por qué el fix de agosto no bastó»**; RUNBOOK con el aviso en la sección de contadores; índice y relevo → **Plan 2 · T2.0** | El Plan 3 queda en **8 / 9 con T3.7 BLOQUEADA**. **No se marca completo mientras CE3 siga rojo**: hacerlo sería repetir el error de agosto, que cerró 10/10 con tres gates abiertos |
