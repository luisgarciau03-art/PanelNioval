...
- **Baseline de verificación:** `python -m pytest tests/` → **1,208 passed, 2 skipped** en `main` (al 2026-09-17, con los Planes 1, 4 y 3 cerrados). La rama `perf/gasto-places-minimo` del Plan 2 da **1,221** porque añade 13 tests suyos. ⚠️ **El baseline es POR RAMA:** un gate escrito como «≥ 1,221» es inalcanzable desde una rama basada en `main` hasta que el Plan 2 mergee — comparar siempre contra el baseline de la rama base, nunca contra un número absoluto. Histórico: 314 (2026-08-27, Plan 3 de agosto) → 525 → 955 → 1,193 (T4.8) → **1,208**. ⚠️ **Ojo con el comando**: `pytest.ini` ya trae `addopts = -q`, así que añadir otro `-q` lo convierte en `-qq` y **suprime la línea del resumen** — se ven los puntos y `exit 0`, pero nunca el número. Por eso el comando oficial va **sin** `-q`. 🪤 Y antes de creerte un rojo raro tras cambiar de rama: `find . -name __pycache__ -exec rm -rf {} +` — bytecode viejo ha producido decenas de fallos falsos aquí, incluido uno que *parecía* una regresión de seguridad. Es el ÚNICO baseline oficial. Nada se mergea con la suite en rojo. Nota: importar `app.py` en frío tarda ~100 s (`googleapiclient` + Defender); en caliente ~8 s. **No es pandas**: medido con `sys.modules`, ni pandas ni numpy llegan a cargarse. `pytest.ini` ancla el rootdir al proyecto.
- **Saltos de línea (desde 2026-08-31):** `.gitattributes` declara `* text=auto`. En **disco, en Windows**, los archivos están en **CRLF** —incluidos los `.py`—, así que un reemplazo por patrón escrito con `\n` **no casa y devuelve el texto igual, sin error**: normaliza antes de sustituir y verifica con `grep` **sobre el archivo**. Pero en **git los blobs son LF**, que es lo que reciben el runner de CI y el VPS; antes eso dependía de `core.autocrlf` de cada máquina y ahora es explícito. No confundir las dos cosas: la trampa del reemplazo es real y la afirmación "el repo está en CRLF" es falsa. ⚠️ **Excepción añadida en la T4.3:** `static/js/vendor/** binary`. Sin ella, `text=auto` convertía el Chart.js auto-hospedado a CRLF al hacer checkout en Windows y su sha256 dejaba de coincidir: el test que garantiza que el archivo de terceros es el revisado **fallaba en la máquina del owner y pasaba en el CI**. Un artefacto de terceros no se normaliza nunca.
...
- **Secretos:** nada hardcodeado. `GOOGLE_CREDENTIALS_JSON`, `TELEGRAM_TOKEN`, `TELEGRAM_CHAT_ID` van por variables de entorno en **`/srv/panel/secretos/.env` del VPS** (ya no en Railway). **Pendiente owner:** rotar el token Telegram `8404009072` (expuesto en el historial git; ~14 copias).
- **Ramas:** **nunca** trabajar directo en `main`. ⚠️ **El motivo ya NO es el auto-deploy** (no existe): es que `main` es lo que el owner desplegará, así que un rojo ahí sale al VPS en el siguiente `git pull`. Una rama por plan; PRs con `gh pr create --base main`; merge `--squash` solo con baseline verde y reviews sin CRITICAL/HIGH abiertos. ⚠️ **Con PRs apilados, el `--squash` los descuelga:** en T1.5 se simuló antes de ejecutar y rompía el PR #43 en 11 archivos. Se mergeó con **merge commit**. **Simula el método de merge antes de elegirlo** cuando haya un PR apilado.
- **Antes de tocar el panel (CSS, marcado o un componente):** lee `docs/diseno/sistema.md`. No es una guía aspiracional — **cada regla salió de un fallo medido**, y varias no las ve ningún `grep`: la opacidad que rompe el contraste sin tocar el color, el par que pasa sobre blanco y no sobre un tinte, el esqueleto que borra los datos recién llegados. Trae la tabla de duraciones y curvas, cómo añadir un componente o un estado de carga, **lo que no se toca sin medir** y la deuda conocida de `dashboard.css` y `formulario.css`.
...
**Tanda 2026-08-27, validada y ampliada el 2026-08-28** (6 planes, 53 tareas, **45 hechas**).
...
| 4 — Rediseño del panel | **12/12 ✅** | Movimiento, presentación, estados de carga, accesibilidad y responsive en las 3 superficies. Rama `feat/rediseno-panel`, PR #43. Sistema documentado en `docs/diseno/sistema.md`. **11 de 12 criterios cumplidos**; CE1 queda abierto y es la decisión D6 del owner |
