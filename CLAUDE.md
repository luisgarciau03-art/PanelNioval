...
- **`worker_catalogo_run.py`** (Plan 5): runner del worker local (Selenium + heartbeat + lock) que el owner corre en su PC (`instalar-worker.ps1` = Tarea Programada). Operación: `docs/RUNBOOK.md`; decisión de transporte: `docs/adr/2026-08-13-transporte-catalogo.md`. Smoke test: `tools/smoke_railway.py`.
- **Autenticación opcional (M1):** si `PANEL_DASHBOARD_TOKEN` está en env, todas las rutas exigen el token (header `X-Dashboard-Token`, `?token=`, o cookie de sesión). Por defecto (sin env) el panel queda abierto — activarlo es acción del owner.
...
- **Baseline de verificación:** `python -m pytest tests/ -q` → **144 passed** (Plan 1 → Plan 5, tanda 2026-08-13). Es el ÚNICO baseline oficial. Nada se mergea con la suite en rojo. Nota: importar `app.py` en frío tarda ~2 min (pandas/googleapiclient + Defender); en caliente ~8s. `pytest.ini` ancla el rootdir al proyecto.
