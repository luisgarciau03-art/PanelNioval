...
- **Datos personales:** teléfonos/nombres/correos de clientes **no** se commitean ni se vuelcan completos en logs; enmascarar (`+52...XXXX`). Desde el 2026-08-31 lo vigila `tests/test_pii_repositorio.py`: el proyecto usaba el teléfono de un **cliente real** como "número canónico de ejemplo" en 21 sitios, uno de ellos visible en la interfaz. ⚠️ El barrido de secretos del CI **no lo habría atrapado**: su patrón exige 10 dígitos contiguos, así que ve `5551234567` pero no `555 123 4567`, `555-123-4567` ni `(555) 123 4567` — y el formato con espacios es justo el que usa la hoja, o sea el que acaba copiado a mano. Endurecer ese patrón es trabajo pendiente del Plan 0. `.gitignore` cubre `*.json` (credenciales) y `debug_invalid_*`/`debug_timeout_*` (screenshots con PII de `envio_catalogo.py`).
