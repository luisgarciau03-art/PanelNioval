...
- **Baseline de verificación:** `python -m pytest tests/ -q` → **137 passed** (Plan 1 → Plan 4, tanda 2026-08-13). Es el ÚNICO baseline oficial. Nada se mergea con la suite en rojo. Nota: importar `app.py` en frío tarda ~2 min (pandas/googleapiclient + Defender); en caliente ~8s. `pytest.ini` ancla el rootdir al proyecto.
